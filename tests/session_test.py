...
from schedule import Session, Interval, CalculatedSession
...
class TestSessionInPeriod(TestSession):
  ...
  def test_2(self):
    ...
    result = self.ses_p.__contains__(datetime.datetime(2011,9,27,15,40), True)
    assert result == result_expected, "bad result ? got %s instead of %s" % (result, result_expected)
  ...
  def test_4(self):
    ...
    result = self.ses_p.__contains__(datetime.datetime(2011,9,28,15,40), True)
    assert result == result_expected, "bad result ? got %s instead of %s" % (result, result_expected)
